...
async function createPost(
	...
	try {
		...
		let tagsInput:
			| {
					create: {
						post_app_tag: { connect: { id: bigint; name: string } };
					}[];
			  }
			| undefined;
		if (Array.isArray(data.tags)) {
			...
			const tagConnections = await Promise.all(
				data.tags.map(async (tagName: string) => {
					...
					return {
						post_app_tag: {
							connect: { id: tag.id, name: tag.name },
						},
					};
				})
			);
		...
		const newPost = await prisma.post.create({
			data: postData,
			include: {
				post_app_post_images: { select: { post_app_image: true } },
				post_app_post_tags: { include: { post_app_tag: true } },
			},
		});
...
async function editPost(
	...
	try {
		...
		if (data.tags && Array.isArray(data.tags)) {
			...
			const currentTagNames = currentPost.post_app_post_tags.map(
				(t) => t.post_app_tag.name
			);
			...
			if (tagsToAdd.length > 0) {
				const tagConnections = await Promise.all(
					tagsToAdd.map(async (tagName: string) => {
						...
						return { post_app_tag: { connect: { id: tag.id } } };
					})
				);
		...
		if (data.images) {
			...
			const currentImages = currentPost.post_app_post_images;

			const imagesToDelete = currentImages.filter((currentImg) => {
				if (data.images)
					return data.images.some(
						(newImg) => newImg.id === currentImg.post_app_image.id
					);
			});
			...
			updateData.post_app_post_images = {
		...
		const updatedPost = await prisma.post.update({
			...
			data: updateData,
			include: {
				post_app_post_images: { include: { post_app_image: true } },
				post_app_post_tags: { include: { post_app_tag: true } },
			},
		});
		...
		const normalizedPost = {
			...updatedPost,
			images: updatedPost.post_app_post_images.map((img) => {
				const relativeUrl = img.post_app_image.filename
					.replace(/\\/g, "/")
					.replace(/^uploads\/+/, "");
				const fullUrl = img.post_app_image.filename.startsWith("http")
